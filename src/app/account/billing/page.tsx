...
export default function Billing() {
    ...
    const { data: paymentData, isLoading: paymentDataLoading, isError: isPaymentDataError, error: paymentDataError } = useGetPayments()
    const { data: paymentMethods, isLoading: paymentMethodsLoading, isError: isPaymentMethodsError, error: paymentMethodsError } = useGetPaymentMethods()
    ...
    useEffect(() => {
        if (ownerData) fetchPrefs(ownerData)
    }, [ownerData])
                            ...
                            <div className="content-body mt-4 flex h-full w-full flex-col gap-4 rounded-sm border border-neutral-800 p-4">
                                <ul className="content-body flex w-full flex-col gap-2">
                                    <li className="flex items-center justify-between">
                                        ...
                                        {!ownerDataLoading ? (
                                            <strong className="capitalize">{ownerData.plan['name']}{ownerData?.cancelled_plan ? " (Cancelled)": ""}</strong>
                                        ) : (
                                    ...
                                    <li className="flex items-center justify-between">
                                        ...
                                        {!ownerDataLoading ? (
                                            <strong>{'£' + ownerData.plan['price']}</strong>
                                        ) : (
                                    ...
                                    </li>
                                    {
                                        !ownerData?.cancelled_plan ? <li className="flex items-center justify-between">
                                            ...
                                            {!ownerDataLoading ? (
                                                <strong>{ownerData.is_verified ? lib.formatDate(ownerData.verified_month_end) : 'N/A'}</strong>
                                            ) : (
                                ...
                                <ul className="content-body flex w-full flex-col gap-2">
                                    <li className="flex items-center justify-between">
                                        ...
                                        {!ownerDataLoading ? (
                                            <strong>{ownerData.current_tokens}</strong>
                                        ) : (
                                    ...
                                    <li className="flex items-center justify-between">
                                        ...
                                        {!ownerDataLoading ? (
                                            <strong>{Math.floor(ownerData.current_tokens / 12)}</strong>
                                        ) : (
