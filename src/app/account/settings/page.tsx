...
const switches = [
    { name: "Auto-citations", checked: true, type: "pref" },
    { name: "Emergency AI rewrite", checked: true, type: "pref" },
    { name: "Auto-removal", checked: true, type: "pref" },
    { name: "Widget", checked: true, type: "ui" },
    { name: "Watermarks", checked: true, type: "ui" },
]
...
export default function Settings() {
    ...
    const handleSwitchesDefault = () => {
        ...
        const vals = [true, false, false, true, false]

        const newSwitches = switchDefaults.map((val, key) => {
            return { name: val.name, checked: vals[key], type: val.type }
        })
                ...
                <ThemeProvider theme={mainTheme}>
                    <div className='grid grid-cols-2 gap-6'>
                        <form className="flex flex-col gap-2 bento-card py-4 my-8" onSubmit={(e) => handleSubmit(e)}>
                            <h2 className='content-subtitle my-2 text-center'>Preferences</h2>
                        ...
                        <form className="flex flex-col gap-2 bento-card py-4 my-8" onSubmit={(e) => handleSubmit(e)}>
                            <h2 className='content-subtitle my-2 text-center'>UI</h2>
                            <div className="h-[2px] w-full opacity-30 bg-gradient-to-r from-transparent via-[#d8af41] to-transparent rounded-full" />
                            {
                                switchDefaults.map((val, key) => {
