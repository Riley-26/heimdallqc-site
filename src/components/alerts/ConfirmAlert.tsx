...
export const ConfirmAlert: React.FC<ConfirmAlert> = ({ isOpen, onClose, hasConfirmed, children, title }) => {
    ...
    return (
        <div className={`fixed inset-0 z-100 flex items-start justify-center mt-16`}>
            {/* Overlay */}
            <div className="fixed inset-0 bg-black/70 transition-opacity" aria-hidden="true" onClick={onClose} />
            {/* Modal */}
            <div className={`bento-card relative z-10 mx-auto w-full max-w-md rounded-lg bg-white p-6 shadow-lg transform transition-all duration-300 ease-in-out ${show ? "translate-y-0 opacity-100" : "-translate-y-8 opacity-0"}`}>
                <button
